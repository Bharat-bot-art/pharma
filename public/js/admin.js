(function () {
  ...
  if (usersBody) {
    (async () => {
      const { ok, data } = await api('/api/admin/users');
      if (!ok) return;
      usersBody.innerHTML = data.users.length === 0
        ? '<tr><td colspan="7" style="text-align:center;padding:20px;color:var(--ink-400)">No users found.</td></tr>'
        : data.users.map(u => `
          <tr>
            <td><strong>${esc(u.full_name)}</strong></td>
            <td>${esc(u.email)}</td>
            <td>${esc(u.mobile || '—')}</td>
            <td><span class="chip" style="text-transform:uppercase;font-size:10px">${esc(u.role)}</span></td>
            <td class="text-xs text-muted">${new Date(u.created_at).toLocaleDateString('en-IN')}</td>
            <td>
              <span class="chip ${u.is_active ? 'chip-green' : 'chip-warn'}">
                ${u.is_active ? 'Active' : 'Disabled'}
              </span>
            </td>
            <td>
              <button class="btn btn-outline btn-sm" data-user-toggle="${u.id}" data-active="${u.is_active ? 'true' : 'false'}">
                ${u.is_active ? 'Disable' : 'Enable'}
              </button>
            </td>
          </tr>
      ...
      usersBody.addEventListener('click', async (e) => {
        ...
        if (toggleBtn) {
          ...
          if (res.ok) {
            window.BIOSYM.toast.success('User status updated');
            setTimeout(() => window.location.reload(), 600);
          } else {
            window.BIOSYM.toast.error('Failed to update status');
          }
        }
      });
    })();
  ...
  if (couponsBody) {
    ...
    window.openCouponModal = (id = null) => {
      ...
      if (id) {
        ...
        if (c) {
          couponForm.elements['id'].value = c.id;
          couponForm.elements['code'].value = c.code;
          couponForm.elements['type'].value = c.type;
          couponForm.elements['value'].value = c.value;
          couponForm.elements['minSubtotal'].value = c.min_subtotal || '';
          couponForm.elements['maxDiscount'].value = c.max_discount || '';
          couponForm.elements['usageLimit'].value = c.usage_limit || '';
          if (c.expires_at) {
            couponForm.elements['expiresAt'].value = new Date(c.expires_at).toISOString().slice(0, 16);
          }
          couponForm.elements['isActive'].checked = !!c.is_active;
          document.querySelector('[data-coupon-modal-title]').textContent = 'Edit Coupon';
        }
      } else {
        couponForm.elements['id'].value = '';
        couponForm.elements['type'].value = 'percent';
        couponForm.elements['isActive'].checked = true;
        document.querySelector('[data-coupon-modal-title]').textContent = 'Add Coupon';
      }
      couponModal.style.display = 'flex';
    };

    document.querySelectorAll('[data-close-coupon-modal]').forEach(btn => {
      btn.addEventListener('click', () => couponModal.style.display = 'none');
    });
    
    couponForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const id = couponForm.elements['id'].value;
      const payload = {
        code: couponForm.elements['code'].value,
        type: couponForm.elements['type'].value,
        value: couponForm.elements['value'].value,
        min_subtotal: couponForm.elements['minSubtotal'].value,
        max_discount: couponForm.elements['maxDiscount'].value,
        usage_limit: couponForm.elements['usageLimit'].value,
        expires_at: couponForm.elements['expiresAt'].value || null,
        is_active: couponForm.elements['isActive'].checked
      };
      
      const res = await api(id ? \`/api/admin/coupons/\${id}\` : '/api/admin/coupons', {
        method: id ? 'PATCH' : 'POST',
      ...
      if (res.ok) {
        window.BIOSYM.toast.success(id ? 'Coupon updated' : 'Coupon created');
        setTimeout(() => window.location.reload(), 600);
      } else {
        const errSlot = document.getElementById('coupon-error-slot');
        if (errSlot) errSlot.innerHTML = \`<div class="alert alert-danger" style="margin-bottom:15px">\${esc(res.message || 'Error saving coupon')}</div>\`;
        else window.BIOSYM.toast.error(res.message || 'Error saving coupon');
      }
    });
    ...
    (async () => {
      ...
      couponsBody.innerHTML = allCoupons.length === 0
        ? '<tr><td colspan="8" style="text-align:center;padding:20px;color:var(--ink-400)">No coupons found.</td></tr>'
        : allCoupons.map(c => `
          <tr>
            <td><strong>${esc(c.code)}</strong></td>
            <td><span style="text-transform:uppercase;font-size:11px">${esc(c.type)}</span></td>
            <td><strong>${c.type === 'percent' ? c.value + '%' : money(c.value)}</strong></td>
            <td>${c.min_subtotal > 0 ? money(c.min_subtotal) : '—'}</td>
            <td>${c.used_count} ${c.usage_limit ? '/ ' + c.usage_limit : ''}</td>
            <td class="text-xs text-muted">${c.expires_at ? new Date(c.expires_at).toLocaleDateString('en-IN') : 'Never'}</td>
            <td>
              <span class="chip ${c.is_active ? 'chip-green' : 'chip-warn'}">
                ${c.is_active ? 'Active' : 'Disabled'}
              </span>
            </td>
            <td>
              <div class="row" style="gap:6px">
                <button class="btn btn-outline btn-sm" onclick="window.openCouponModal(${c.id})">Edit</button>
                <button class="btn btn-danger btn-sm" data-coupon-del="${c.id}">Delete</button>
              </div>
            </td>
          </tr>
        `).join('');
      ...
      couponsBody.addEventListener('click', async (e) => {
        ...
        if (delBtn) {
          ...
          const res = await api(\`/api/admin/coupons/\${delBtn.dataset.couponDel}\`, { method: 'DELETE' });
          if (res.ok) {
            window.BIOSYM.toast.success('Coupon deleted');
